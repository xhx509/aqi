# -*- coding: utf-8 -*-
"""
Created on Mon May  9 14:42:59 2016
...
CHUNKSIZE = 2 ** 20 * 256  # 256 MiB chunks
...
class Gdcp(object):
    ...
    def print_failed(self):
        if len(self.failures["HTTP"]):
            failures = "\n".join([f.title for f in self.failures["HTTP"]])
        ...
        if len(self.failures["MD5"]):
            failures = "\n".join([f.title for f in self.failures["MD5"]])
...
def do_setup(location=None):
    if not location:
        location = os.path.join(os.environ["HOME"], "." + PROJ)
    settings = os.path.join(location, "settings.yaml")
    client_secrets = os.path.join(location, "client_secret.json")
...
def cli():
    ...
    # First get rid of message about oauth2client.util not having a handler
    oauth2_util.addHandler(logging.NullHandler())
    do_setup()
...
def cli_download(args):
    ...
    if gdcp.failed():
        ...
        stdoutn("Downloaded %i file(s) and folder(s)" % len(gdcp.files))
        log.info("Downloaded %i file(s) and folder(s)" % len(gdcp.files))
...
def cli_upload(args):
    ...
    if gdcp.failed():
        ...
        stdoutn("Uploaded %i file(s) and folder(s)" % len(gdcp.files))
        log.info("Uploaded %i file(s) and folder(s)" % len(gdcp.files))
...
def cli_transfer_ownership(args):
    ...
    stdoutn("Transferred ownership for %i file(s)" % len(gdcp.files))
